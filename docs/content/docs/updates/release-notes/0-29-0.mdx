...
`satellite-apl` with `metadata` feature enabled now exports the
...
```diff
[dependencies]
satellite-apl = { version = "0.29.0", features = ["metadata"] }
- mpl-token-metadata = "1.13.1"
```

and use the exported crate from `satellite-apl`:

```rust
use satellite_apl::metadata::mpl_token_metadata;
```
