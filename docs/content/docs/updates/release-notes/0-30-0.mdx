...
to be specified in the list of `idl-build`, e.g. `satellite-apl/idl-build`,
...
[documentation](https://docs.rs/satellite-lang/0.30.0/satellite_lang/macro.declare_program.html).
...
`satellite-apl` now includes CPI wrappers for Token Extensions which can be
accessed from `satellite_apl::token_2022_extensions`.
...
Using non-matching versions of `anchor-cli`, `satellite-lang`, and
