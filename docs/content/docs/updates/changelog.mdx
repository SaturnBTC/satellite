...
- cli: Warn if `satellite-apl/idl-build` is missing
...
- lang: Add `satellite_lang::pubkey` macro for declaring `Pubkey` const values
...
- ts: Add `.interface(..)` method for instruction discriminator overrides
  ([#2728](https://github.com/coral-xyz/anchor/pull/2728)).
- cli: Check `satellite-lang` and CLI version compatibility
...
- lang: `Box` the inner enums of `satellite_lang::error::Error` to optimize
...
- spl: Add `idl-build` feature. IDL build method will not work without enabling
  this feature when using `satellite-apl`
  ([#2629](https://github.com/coral-xyz/anchor/pull/2629)).
...
- lang: Add `PartialEq` and `Eq` for `satellite_lang::Error`
...
- lang: add
  [`error!`](https://docs.rs/satellite-lang/latest/satellite_lang/prelude/macro.error.html)
  and
  [`err!`](https://docs.rs/satellite-lang/latest/satellite_lang/prelude/macro.err.html)
  ...
  anchor internal errors). _ change all `arch_program::program::invoke()` to
  `arch_program::program::invoke().map_err(Into::into)` and
  `arch_program::program::invoke_signed()` to
  `arch_program::program::invoke_signed().map_err(Into::into)`
...
- lang: `ProgramData` and `UpgradableLoaderState` can now be passed into
  ...
  [UpgradeableLoaderState](https://docs.rs/solana-program/latest/arch_program/bpf_loader_upgradeable/enum.UpgradeableLoaderState.html).
...
- lang: Add `satellite-debug` feature flag for logging
